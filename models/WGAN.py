# WGAN
# WGAN 비평자는 진짜 이미지에 대한 점수를 높임으로써 진짜 이미지와 생성된 이미지에 대한 예측 사이의 차이를 최대화
# WGAN의 비평자와 생성자를 훈련하는 모델을 컴파일할 때 이진 크로스엔트로피 대신 와서스테인 손실을 지정할 수 있다.
# WGAN 비평자의 마지막 층에는 시그모이드 활성화 함수가 필요하지 않다.
# 매 업데이트 후에 비평자의 가중치 클리핑
# 생성자를 업데이트할 때마다 비평자를 여러번 훈련
...
class WGAN():
    ...
    def train_critic(self, x_train, batch_size, clip_threshold, using_generator):
        ...
        valid = np.ones((batch_size,1))  # 진짜 레이블엔 1.
        fake = -np.ones((batch_size,1))  # 가짜 레이블엔 -1.
        ...
        for l in self.critic.layers:
            ...
            weights = [np.clip(w, -clip_threshold, clip_threshold) for w in weights]  # 가중치 클리핑(가중치 깎음)을 통해 립시츠 제약(두 점 사이의 거리를 일정 비 이상으로 증가시키지 않는 함수)을 부과.
            # 립시츠 제약 : 어느 지점에서 상승하거나 하강하는 비율 한정
            # np.clip(array, min, max) : min보다 작은 값은 min, max보다 큰 값은 max 값으로 바꾼다.
